<?php
/**
 * svAdminBoard base components.
 *
 * @package    plugins
 * @subpackage svAdminBoard
 * @author     Ivan Tanev aka Crafty_Shadow @ webworld.bg <vankata.t@gmail.com>
 * @version    SVN: $Id: BasesvAdminBoardComponents.class.php 25335 2009-12-14 12:49:08Z Crafty_Shadow $
 */ 
class BasesvAdminBoardComponents extends sfComponents
{

  /**
  * The main navigation component for the svAdminBoard plugin
  */  
  public function executeHeader()
  {
    ...
    {
      svAdminBoard::setProperty('include_path', false); // we don't render the breadcrumbs when we are in a 404 error module/action
      $this->module_link = null;
      $this->action_link = null;
    }
    else
    {
      if (!svAdminBoard::routeExists($this->module_link = $this->getContext()->getModuleName(), $this->getContext()))
      {
        ...
        foreach (svAdminBoard::getAllItems() as $name => $item) if ($name == $this->getContext()->getModuleName()) { $this->module_link = $item['url']; break; }        
      }

      $this->module_link_name = svAdminBoard::getModuleName($this->getContext()); 
      
      if ($this->getContext()->getActionName() != 'index')
      {
        $this->action_link = $this->getContext()->getRouting()->getCurrentInternalUri();
        $this->action_link_name = svAdminBoard::getActionName($this->getContext());
      }
      else
      {
        $this->action_link = null;
